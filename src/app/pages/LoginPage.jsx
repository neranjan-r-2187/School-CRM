...
import { Eye, EyeOff, User, Lock, ArrowRight, Loader2, Fingerprint } from "lucide-react";
...
export const LoginPage = () => {
  ...
  const roles = ["Parent", "Teacher", "Staff", "Admin"];
  ...
  >
    ...
    className={`flex-1 text-xs font-medium py-2 px-1 rounded-lg transition-all duration-200 relative z-10 ${selectedRole === role ? "text-blue-700" : "text-slate-500 hover:text-slate-700"}`}
  >
                {role}
                {selectedRole === role && <motion.div
    layoutId="role-tab"
  ...
  />}
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="space-y-1">
              <label className="text-xs font-semibold text-slate-600 uppercase tracking-wider ml-1">Email or Username</label>
  ...
  >
          ...
          {selectedRole === "Parent" && <div className="mt-6 flex items-center justify-center gap-2 text-slate-400 text-sm">
